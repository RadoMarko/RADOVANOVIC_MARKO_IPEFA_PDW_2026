...
const apiBaseUrl = 'http://localhost:3000/api/account';
const publicRoutes = [
  `${apiBaseUrl}/signin`,
  `${apiBaseUrl}/signup`,
  `${apiBaseUrl}/refresh`,
...
export const authInterceptor: HttpInterceptorFn = (request, next) => {
  ...
  if (!request.url.startsWith(apiBaseUrl) || publicRoutes.includes(request.url)) {
    return next(request);
  }
  ...
  return next(addTokenInHeader(request, token)).pipe(
    catchError((error: HttpErrorResponse) => {
      ...
      return service.refresh().pipe(
        switchMap((response) =>
          next(addTokenInHeader(request, response.data.token)),
        ),
        catchError(() => {
          service.logout();
          void router.navigateByUrl('/');
